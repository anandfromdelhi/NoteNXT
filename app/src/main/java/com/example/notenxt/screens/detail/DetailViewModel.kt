...
class DetailViewModel(
    ...
    fun onColorChange(colorIndex: Int) {
        detailUiState = detailUiState.copy(colorIndex)
    }
    ...
    fun getNote(noteId: String) {
        repository.getNote(noteId, onError = {}) {
            detailUiState = detailUiState.copy(selectNote = it)
            detailUiState.selectNote?.let { it1 -> setEditFields(it1) }
        }
    }
